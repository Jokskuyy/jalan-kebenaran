...
function teachingPlan(concepts: string[]) {
  return concepts.map((concept, index) => {
    ...
    if (!note) {
      return `${index + 1}. ${concept}\n   Explain the concept in Bahasa Indonesia with a definition, this week's case example, an anti-pattern, and a comprehension question.`;
    }
    return [
      `${index + 1}. ${concept}`,
      `   Definition: ${note.definition}`,
      `   Case example: ${note.example}`,
      `   Common trap: ${note.trap}`,
      `   Check question: ${note.check}`,
    ].join('\n');
  }).join('\n');
}

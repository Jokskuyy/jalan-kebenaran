...
function teachingPlan(concepts: string[]) {
  return concepts.map((concept, index) => {
    const note = teachingNotes[concept];
    if (!note) {
      return `${index + 1}. **${concept}**\n   Arti: jelaskan dalam satu kalimat Bahasa Indonesia.\n   Di kasus ini: hubungkan ke satu fakta atau tugas minggu ini.\n   Waspada: sebutkan satu kesalahan umum.\n   Cek: ajukan satu pertanyaan singkat.`;
    }
    return [
      `${index + 1}. **${concept}**`,
      `   Arti: ${note.definition}`,
      `   Di kasus ini: ${note.example}`,
      `   Waspada: ${note.trap}`,
      `   Cek: ${note.check}`,
    ].join('\n');
  }).join('\n');
...
export function buildMissionPrompt(
  ...
  return [
    ...
    '2. The first response must only orient me and teach the first concept. Do not restate the full dossier, task list, resources, rubric, or later concepts. Do not ask for the final artifact in the first response.',
    '3. Teach exactly one concept per turn. Use only the current concept card; ignore later cards until their turn. After the check question, stop and wait for my answer before teaching the next concept.',
    '4. If I answer incorrectly, correct only the misconception, give one case-specific example, and ask the check question again. Do not reveal the finished answer.',
    '5. After all concepts are checked, ask for a readback in no more than three bullets. Only after the readback, ask me to paste my own draft.',
    '6. A draft may be incomplete bullets. Treat it as my work to review, not permission to write the artifact for me.',
    '7. Ask at most five high-impact discovery questions after I share my draft. Do not bury me in a generic questionnaire.',
    '8. Separate evidence, client claims, assumptions, and your inferences. Never invent missing case facts.',
    '9. Challenge unsupported claims, unsafe boundaries, and metrics without formula, denominator, source, or owner.',
    '10. Score every rubric item from 0–2: 0 = absent or unsafe, 1 = partial, 2 = complete and evidence-backed.',
    '11. Finish with a prioritized revision checklist. Do not rewrite the final artifact for me.',
    '',
    '## INTERMEZZO PROTOCOL',
    'Use this compact format for each concept:',
    '**[Term]** — [arti sederhana dalam Bahasa Indonesia; tulis istilah English dalam kurung bila membantu].',
    '**Di kasus ini:** [satu hubungan konkret ke evidence atau task minggu ini].',
    '**Waspada:** [satu kesalahan umum].',
    '**Cek:** [satu pertanyaan yang harus saya jawab].',
    'Keep each intermezzo under 70 words. Common terms such as user, owner, source, target, data, KPI, API, and JSON need only a short parenthetical gloss when first used, not a separate lesson. Do not add analogy, history, code, framework details, or multiple examples unless I ask for them or say I am confused.',
    'Explain a technical term the first time it blocks the task; do not expand every familiar word into a separate lesson.',
    '',
    '## CASE DOSSIER',
    caseText,
    '',
    ...
    '1. First reply: give a Context snapshot in at most four bullets, state the one outcome for this week, then teach only concept 1 with INTERMEZZO PROTOCOL. Do not preview concepts 2–n or the full task list.',
    '2. Ask the single Cek question and stop. Wait for my answer.',
    '3. On each next turn, correct or confirm my answer, then teach the next concept with one intermezzo. Stop after its Cek question.',
    '4. When all concepts are complete, ask for my three-bullet case-and-concept readback. Do not ask for the deliverable yet.',
    ...
    '',
    '## USER CONTROLS',
    'If I say “jelaskan [term]”, explain only that term in at most three bullets. If I say “beri hint”, give one directional hint without writing the answer. If I say “lanjut”, move to the next concept. If I say “stop”, wait for my next instruction.',
    '',
    '## MY DRAFT',
